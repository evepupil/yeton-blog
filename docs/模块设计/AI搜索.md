...
>
> **对应代码**：`functions/api/ai-search.ts`、`lib/ai-search/`、`features/ai-search/`、`site.config.ts`、`wrangler.jsonc`
>
...
| `lib/ai-search/`                 | 请求、SSE、累计文本和引用映射纯函数 |
...
2. AutoRAG 发送的是累计回答。服务端记录上一版文本，只把新增部分作为 `delta` 发给浏览器，避免内容重复。
...
5. 接口最多接收 500 个 Unicode 字符。每个来源每分钟最多 6 次，全站每分钟最多 30 次，调用和读取响应都受 30 秒超时约束。
6. Function 不记录问题、IP 或回答正文。日志只写请求 ID、阶段、耗时、错误码和引用数量。
7. Drawer 在读者第一次点击浮动按钮后才加载。关闭 Drawer 或点击停止会取消浏览器请求，并继续保留已收到的部分文字。
...
- AI、用户限流或全站限流 binding 缺失时返回 `503`。达到限流时返回 `429` 和 `Retry-After: 60`。
...
单测覆盖输入、同源校验、限流顺序、跨分片 SSE、累计文本、引用映射、结构化错误和客户端完成条件。Playwright 拦截 `/api/ai-search`，覆盖成功回答、停止、失败重试和点击引用。

生产发布后还需真实请求 `/api/ai-search`，确认 Cloudflare 项目识别三个 binding，AutoRAG 返回带引用的回答，并检查日志没有问题或 IP 正文。完成这一步后才能把 M8 标记为“已完成”。

## 待扩展项

- 根据真实 AutoRAG 来源字段补充更精确的文章标题元数据。
- 观察一段时间的限流命中和额度使用后，再调整每用户与全站阈值。
...
- 增加严格类型的 SSE 协议、累计文本转换与旧 slug 引用映射。
