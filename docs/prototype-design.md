...
这份原型用于确认个人博客的页面结构、功能入口与交互方式。视觉采用独立设计，功能范围参考 `D:\myproject\notion-fuwari`。

## 功能范围

### 原型已覆盖

- 首页作者介绍、社交入口、文章标签、置顶/近期文章
- 文章列表、标签筛选、文章数量
- 全文搜索，覆盖标题、标签、摘要与正文
- 按年份归档与标签过滤
- 图书列表、阅读进度与章节目录
...
- 独立的数据型关于页原型，覆盖个人资料、GitHub 热力图、TokenBoard 用量、微信读书和技术雷达
...
- 首页布局：左侧为博客定位与主要入口，右侧纵向排列个人资料卡和标签卡，近期文章直接衔接首屏
- 图片：个人资料卡使用本地头像，文章卡片使用统一裁切的实景图片
...
| 首页首屏 | `HomeHero`, HeroUI `Card`, `Chip`, `Link`           |
...
数据型关于页使用独立的 `prototype/about.html`、`prototype/about.css` 和 `prototype/about.js`，避免影响早期多页面原型。该页面中的活动与阅读数值均为清楚标注的示意数据，只用于确认信息层级、图表密度和响应式布局；正式页面再接入 GitHub、TokenBoard 与微信读书数据源。

2026-07-20 用户确认该原型的整体质感与信息结构，同时要求删除 Hero 和活动数据之间的深色“最近关注”横条。原型与正式页面均改为用细分隔线直接衔接“最近 365 天”，页面保持纯白、开放的纵向节奏。
